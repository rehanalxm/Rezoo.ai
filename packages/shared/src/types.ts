// ============================================================
// @rezoo/shared — Core type definitions
// ============================================================

/** Current state of the Rezoo assistant */
export type RezooState =
  | 'idle'
  | 'listening'
  | 'thinking'
  | 'speaking'
...
export type ClientMessage =
  ...
  | { type: 'stop_listening' }
  | { type: 'cancel_current' } // barge-in cancellation
  | { type: 'confirm_action'; actionId: string; confirmed: boolean }
  | { type: 'text_input'; text: string }; // text fallback

/** Server → Client messages */
export type ServerMessage =
  | { type: 'state_change'; state: RezooState }
  | { type: 'transcript_partial'; text: string }
  | { type: 'transcript_final'; text: string }
  | { type: 'response_text'; text: string }
  | { type: 'audio_playback'; data: string } // base64-encoded audio
  | { type: 'audio_playback_done' }
  | { type: 'stop_playback' } // barge-in: stop current TTS playback
  | { type: 'tool_start'; toolName: string; description: string }
  | { type: 'tool_result'; toolName: string; success: boolean; message: string; data?: any }
  | { type: 'open_url'; url: string; title?: string }
  | { type: 'confirm_request'; actionId: string; toolName: string; description: string; params: Record<string, unknown> }
